## TODO - 
ACTIVITYLEVELS = {"none": 1.2, "little": 1.375, "moderate":1.55, "active":1.725, "high":1.9}
# Fitness app
class FitnessApp:
    ...
    def _bmrCalc(self, info: dict) -> float:
        ...
        match info["gender"].lower():
            case "male":
                bmr = ((13.397*info["weight"]) + (4.799*info["height"]) - (5.677*info["age"])) + 88.362
            case "female":
                bmr = ((9.247*info["weight"])+ (3.098*info["height"])- (4.330*info["age"])) + 447.593
        return round(bmr, 2)
    ...
    def setMacros(self) -> dict:
        ...
        fats = 0.4*poundBW
        print(fats)
        carbs = round((self.target - (proteins*4) - (fats*9))/4)


        return {"proteins": [proteins, proteins*4], "fats": [fats, fats*9], "carbs": [carbs, carbs*4]}
